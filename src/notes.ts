...
export class NoteWriter {
	...
	async writeThreadIndex(vaultSubfolder: string, threads: ThreadEntry[]): Promise<void> {
		...
		const lines: string[] = [
			...
			"",
			`> ${threads.length} conversation(s). Auto-generated by the Outlook Teams and Calendar plugin.`,
			"",
		];
	...
	async writeConversationIndex(subfolder: string, convos: TeamsConversation[]): Promise<void> {
		...
		const lines: string[] = [
			...
			"# Teams Conversations",
			"",
			`> ${convos.length} conversation(s). Auto-generated by the Outlook Teams and Calendar plugin.`,
			"",
		];
